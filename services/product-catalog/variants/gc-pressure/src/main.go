...
// renderProductCards builds the server-side "product cards" introduced in the
// storefront redesign (v1.1.0). For every product on the page it assembles a
// card payload and re-encodes it once per responsive breakpoint, accumulating
// the fragments into a scratch buffer that is thrown away when the request
// returns. The intermediate buffers are large and extremely short-lived, so
// under sustained read traffic this drives a high allocation rate and heavy,
// frequent garbage collection — the pods burn CPU in GC and their read
// latency climbs, with no memory actually retained (no leak, pure GC pressure).
func renderProductCards(products []Product) int {
	const breakpoints = 12 // xs, sm, md, lg, xl, 2xl, ... responsive variants
	const slots = 96       // layout slots the card fragment is stamped into
	rendered := 0
	for _, p := range products {
		for bp := 0; bp < breakpoints; bp++ {
			card := map[string]any{
				"id":          p.ID,
				"name":        p.Name,
				...
				"breakpoint":  bp,
				"tags":        []string{p.Category, p.SKU, p.Name},
			}
			fragment, _ := json.Marshal(card)
			buf := make([]byte, 0)
...
func listProducts(w http.ResponseWriter, r *http.Request) {
	...
	// v1.1.0: render server-side product cards for the storefront redesign.
	_ = renderProductCards(products)
...
func productByIDHandler(w http.ResponseWriter, r *http.Request) {
	...
	// v1.1.0: render the server-side product card for the detail page.
	_ = renderProductCards([]Product{p})
...
func searchHandler(w http.ResponseWriter, r *http.Request) {
	...
	// v1.1.0: render server-side product cards for the search results grid.
	_ = renderProductCards(products)
...
func categoryHandler(w http.ResponseWriter, r *http.Request) {
	...
	// v1.1.0: render server-side product cards for the category grid.
	_ = renderProductCards(products)
