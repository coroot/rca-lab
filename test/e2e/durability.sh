#!/usr/bin/env bash
# End-to-end durability drills for the MaintenanceJob operator.
#
...
phase()   { kubectl get maintenancejob "$1" -n $ns -o jsonpath='{.status.phase}' 2>/dev/null; }
...
kubectl patch maintenancejob pg-analytics-queries -n $ns --type=merge -p '{"spec":{"enabled":true}}' >/dev/null
if wait_for 90 '[ "$(phase pg-analytics-queries)" = Active ] && [ "$(jobcount analytics-reporting)" = 1 ]'; then
    kubectl delete maintenancejob pg-analytics-queries -n $ns --timeout=120s >/dev/null
    if [ "$(jobcount analytics-reporting)" = 0 ] && [ -z "$(kubectl get maintenancejob pg-analytics-queries -n $ns --no-headers 2>/dev/null)" ]; then
...
kubectl patch maintenancejob traffic-spike -n $ns --type=merge -p '{"spec":{"enabled":true}}' >/dev/null
if wait_for 60 '[ "$(replicas load-generator)" = 5 ]'; then
    kubectl delete pod -n $ns -l app=maintenance-controller --wait=false >/dev/null
    kubectl patch maintenancejob traffic-spike -n $ns --type=merge -p '{"spec":{"enabled":false}}' >/dev/null
...
metadata:
  name: ghost-workload
  namespace: default
  labels: { maintenance.platform.dev/job: ghost-scenario, app: ghost-workload }
spec:
...
EOF
kubectl rollout restart deployment/maintenance-controller -n $ns >/dev/null
kubectl rollout status deployment/maintenance-controller -n $ns --timeout=120s >/dev/null
